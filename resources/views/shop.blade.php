...
<body class="homepage">
  ...
  <form method="GET" action="{{ route('products') }}">
    <div class="filter-box-glass p-4 shadow">
      <h5 class="filter-title-glass mb-4">Filters</h5>

      <!-- Price Range -->
      <div class="filter-section mb-4">
        <h6 class="section-title-glass">Price Range (₹)</h6>
        <div id="price-slider" class="mb-3"></div>

        <div class="d-flex gap-2">
          <input type="number" id="input-min" name="min_price" class="form-control form-control-sm glass-input" placeholder="Min ₹" value="{{ request('min_price') }}">
          <input type="number" id="input-max" name="max_price" class="form-control form-control-sm glass-input" placeholder="Max ₹" value="{{ request('max_price') }}">
        </div>
      </div>
      ...
      <div class="d-grid gap-2">
        ...
        <a href="{{ route('products') }}" class="btn btn-outline-secondary btn-sm rounded-pill">Reset Filters</a>
      </div>
    </div>
  </form>
</aside>
      ...
      <div class="col-md-9">
        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
          <h2 class="h4 mb-0">Shop Our Collection</h2>
          <form method="GET" action="{{ route('products') }}" class="d-flex align-items-center">
            ...
            <select id="sortSelect" name="sort" class="form-select form-select-sm w-auto" onchange="this.form.submit()">
          ...
          @forelse ($products as $product)
            <div class="col-6 col-md-4 col-lg-3">
              <a href="{{ route('product.details', $product->id) }}" class="text-decoration-none">
                <div class="product-card position-relative bg-white rounded overflow-hidden border">
                  <div class="product-img position-relative overflow-hidden">
                    <img src="{{ asset('storage/' . ($product->images->first()->image_path ?? 'images/default.jpg')) }}" class="img-fluid w-100" style="height: 300px; object-fit: cover;" alt="{{ $product->name }}">
                    <a href="#" class="wishlist-icon position-absolute top-0 end-0 m-2 text-dark">
  ...
  // Get values from Blade request helper
  var minPrice = {{ request('min_price', 200) }};
  var maxPrice = {{ request('max_price', 1000) }};

  noUiSlider.create(priceSlider, {
    start: [minPrice, maxPrice], // use server-passed values
    connect: true,
    range: {
      min: 0,
      max: 5000
  ...
  priceSlider.noUiSlider.on('update', function (values, handle) {
    const rawValue = parseInt(values[handle].replace('₹', ''));
    if (handle === 0) inputMin.value = rawValue;
    else inputMax.value = rawValue;
  });
